...
export interface IEditorInspectorNumberFieldProps
  extends IEditorInspectorFieldProps {
  min?: number;
  max?: number;
...
export function EditorInspectorNumberField(
  props: IEditorInspectorNumberFieldProps
) {
  ...
  const ratio = hasMinMax
    ? (
        Scalar.InverseLerp(props.min!, props.max!, parseFloat(value)) * 100
      ).toFixed(0)
    : 0;
            ...
            (mouseUpListener = () => {
              ...
              if (v !== oldV && !props.noUndoRedo) {
                ...
                if (!isNaN(v) && !isNaN(oldV)) {
                  const oldValue = props.asDegrees
                    ? Tools.ToRadians(oldV)
                    : oldV;
