...
export class VFXComponentsPanel extends Component<IVFXComponentsPanelProps> {
					...
					<div className="flex-shrink-0">
						{this._getFilteredComponents().map((component) => (
							<ContextMenu key={component.id}>
								...
								<ContextMenuContent>
									<ContextMenuItem onClick={() => this.props.onComponentSelect(component)}>
										Select
									</ContextMenuItem>
									<ContextMenuSeparator />
									<ContextMenuItem 
										onClick={() => this.props.onComponentRemove(component.id)}
	...
	private _getFilteredComponents() {
		...
		return this.props.vfxData.nodes.filter(component => 
			component.name.toLowerCase().includes(this.props.search.toLowerCase()) ||
			component.type.toLowerCase().includes(this.props.search.toLowerCase())
		);
	}
